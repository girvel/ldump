...
Asynchronous serialization can be achieved through [overloading](/docs/overloading.md#2custompreprocess) `ldump.serializer`: each value node is passed through it, so it's possible to insert a conditional `coroutine.yield()` there. The resulting asynchronicity seems to be stable.
